...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if(path==="/sessions/list") return jsonResponse({success:true,sessions:(await firestoreList(env,`user_sessions/${caller.sub}/devices`)).filter(x=>x.active!==false).sort((a,b)=>(b.lastSeenAt||0)-(a.lastSeenAt||0))});
      if(path==="/sessions/revoke-current") { const id=String(payload.sessionId||"").replace(/[^A-Za-z0-9_-]/g,""); if(id){const old=(await firestoreGet(env,`user_sessions/${caller.sub}/devices/${id}`))||{sessionId:id,uid:caller.sub};await firestoreSet(env,`user_sessions/${caller.sub}/devices/${id}`,{...old,active:false,revokedAt:Date.now()});} return jsonResponse({success:true}); }
